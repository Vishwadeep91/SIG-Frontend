...
const Projects = () => {
    ...
    const fetchProjectApplications = async (projectId) => {
        try {
            ...
            // Filter applications for the current project
            const projectApplications = response.data.filter(app => app.project._id === projectId);
            setApplications(projectApplications);

            console.log('Fetched applications:', projectApplications); // For debugging
        } catch (err) {
            console.error('Error fetching applications:', err);
            toast.error('Failed to load project applications');
        } finally {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            formData.append('teamLead', projectForm.teamLead._id); // Send only the ID
            ...
            toast.error(err.response?.data?.message || `Failed to ${isEditMode ? 'update' : 'create'} project`);
        } finally {
            setSubmitting(false);
        }
    };
        ...
        try {
            ...
            const response = await axios.post(
                `${BaseUrl}/project-applications/apply`,
                {
                    ...
                    resumeOrPortfolio: applicationForm.resumeOrPortfolio
                },
                {
                    headers: { 'Authorization': `Bearer ${token}` }
                }
