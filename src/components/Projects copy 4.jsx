...
const Projects = () => {
    ...
    const fetchProjectDetails = async (projectId) => {
        try {
            ...
            // Fetch applications whenever project details are loaded
            await fetchProjectApplications(projectId);
            // Check if user has applied
            setHasApplied(checkIfUserHasApplied(projectId));
        } catch (err) {
    ...
    const fetchProjectApplications = async (projectId) => {
        try {
            ...
            // Filter applications for the current project for both admin and non-admin users
            const projectApplications = response.data.filter(app => app.project._id === projectId);
            setApplications(projectApplications);

            // Update hasApplied state
            const userHasApplied = projectApplications.some(app => !app.droppedByAdmin);
    ...
    const handleApplySubmit = async (e) => {
        ...
        try {
            ...
            console.log('Application submission response:', applicationResponse.data);

            toast.success('Application submitted successfully!');
            setApplyModalOpen(false);
            setApplicationForm({ resumeOrPortfolio: '' });
            // Refresh applications list and update hasApplied
                                                            ...
                                                            <Button
                                                                ...
                                                                onClick={() => {
                                                                    const application = applications.find(app =>
                                                                        app.project._id === selectedProject._id && !app.droppedByAdmin
                                                                    );
                                                                    if (application) {
